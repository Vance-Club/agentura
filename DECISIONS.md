...
## DEC-101: Heartbeat is notify-only — no auto-triggering of skills (2026-03-18)
**Chose**: Heartbeat posts notification to observe channel listing due skills; users trigger manually
**Over**: Auto-triggering due skills from heartbeat runner
**Why**: Auto-triggered skills posted top-level messages to their default channel (#ops-tech-issues) without thread context, causing noise. Skills don't receive thread_ts from heartbeat, so they can't thread-reply. 5 iterations of gateway fixes before realizing the architecture was wrong.
**Constraint**: Users must manually trigger skills from the observe channel; heartbeat output format unchanged (still returns `{due, message}` JSON)
